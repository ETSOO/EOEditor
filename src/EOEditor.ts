...
export class EOEditor extends HTMLElement implements IEOEditor {
  ...
  private popupTextbox(div?: HTMLDivElement) {
    ...
    fileInput.addEventListener("change", () => {
      ...
      DomUtils.fileToDataURL(file).then((data) => {
        ...
        if (bgImageArea.value) {
          items.push(
            ...bgImageArea.value.split(/\s*,\s*(?=(url)|(linear-gradient))/gi)
          );
        }
        items.push(`url(${data})`);
    ...
    if (bgImageArea.value) {
      ...
      const item = items.find((item) => item.startsWith("linear-gradient"));
      if (item) {
        const itemPos = item.indexOf("(");
        const itemSettings = item.substring(itemPos + 1, item.length - 1);
    ...
    this.popup
      ...
      ?.addEventListener("click", () => {
        ...
        this.setPopupStyleValue(style, "float", 'input[name="float"]:checked');
  ...
  tableProperties(table: HTMLTableElement) {
    ...
    this.popup
      ...
      ?.addEventListener("click", () => {
        ...
        const caption =
          this.popup.querySelector<HTMLTextAreaElement>("#caption")?.value;
        if (caption) {
        ...
        const borderWidth =
          this.popup.querySelector<HTMLInputElement>("#borderTopWidth")?.value;
        let tableBorder =
          this.popup.querySelector<HTMLInputElement>("#tableBorder")?.value;
        if (tableBorder && /0[a-z]{0, 2}/i.test(tableBorder)) tableBorder = "";
        ...
        const tableBorderCollapse = this.popup.querySelector<HTMLInputElement>(
          "#tableBorderCollapse"
        )?.checked;
        Reflect.set(
          style,
  ...
  private popupVideo() {
    ...
    const getCodeInput = () =>
      this.popup.querySelector<HTMLTextAreaElement>('textarea[name="code"]')!;
    ...
    this.popup
      ...
      .forEach((button, key) => {
        button.addEventListener("click", () => {
          ...
          items.querySelectorAll<HTMLDivElement>("div").forEach((div) => {
            div.addEventListener("click", () => {
              this.insertHTML(div.innerHTML, true);

              const num = parseInt(div.title.substring(2), 16);
              ...
              if (index !== 0) {
                ...
                // Desktop max items
                if (latestCharacters.length > 22) latestCharacters.pop();
  ...
  private iframe(element?: HTMLIFrameElement) {
    ...
    this.popup
      ...
      ?.addEventListener("click", () => {
        ...
        const height =
          this.popup.querySelector<HTMLInputElement>("#height")?.value;
        const allowfullscreen = this.popup.querySelector<HTMLInputElement>(
          'input[name="allowfullscreen"]'
        )?.checked;
        const border =
          this.popup.querySelector<HTMLInputElement>("#border")?.valueAsNumber;
        ...
        if (element) {
          ...
          this.insertHTML(
            `<iframe src="${src}" width="${width}" height="${height}" frameborder="${
              border ?? 0
            }px"${allowfullscreen ? " allowfullscreen" : ""}></iframe>`,
            false
          );
        }
  ...
  private tableSplitCell(callback: (isRow: boolean, qty: number) => void) {
    ...
    this.popup
      ...
      ?.addEventListener("click", () => {
        ...
        const qtyInput = this.popup.querySelector<HTMLInputElement>("#qty")!;
  ...
  private toggleSource() {
    ...
    if (isSource) {
      this.editorSourceArea.value = this.editorWindow.document.body.innerHTML;
    } else {
      this.editorWindow.document.body.innerHTML = this.editorSourceArea.value;
      styles.reverse();
    }
  ...
  insertImage() {
    ...
    const doCurrentItem = () => {
      previewDiv.querySelectorAll("img").item(index).style.display = "none";
    };
    ...
    const urlInput = this.popup.querySelector<HTMLTextAreaElement>("textarea")!;
    const pButton =
      this.popup.querySelector<HTMLButtonElement>("button#previous")!;
    pButton.addEventListener("click", () => doPrevious());
    const nButton = this.popup.querySelector<HTMLButtonElement>("button#next")!;
  ...
  executeCommand(name: EOEditorCommandKey) {
    ...
    if (name.startsWith("h") && name.length === 2) {
      // Better than 'heading'
      if (name === "hp") return doc.execCommand("formatBlock", false, "p");
