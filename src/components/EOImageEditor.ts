...
export class EOImageEditor extends HTMLElement {
  ...
  private setup() {
    ...
    let objectLastPos: [FabricType.FabricObject, number?, number?] | undefined;
    ...
    fc.on("mouse:dblclick", () => {
      if (
        ...
        const imageInput =
          this.icons.querySelector<HTMLInputElement>("input#imageFile");
        imageInput?.click();
      }
    });
  }
  ...
  private updateStatus() {
    ...
    if (o == null) {
      ...
      ["hcenter", "vcenter", "bringToBack", "bringToFront", "delete"].forEach(
        (n) => {
          const b = this.icons.querySelector<HTMLButtonElement>(
            `button[name="${n}"]`
          );
